...
setRewardAccount("zombrox@pm.me")
if (ImageCache) then
...
function SingleImageWait(target, waitTime, boxRegion, Similarity, Color, Mask)
    -- Added a default waitTime if not provided (e.g., 2 seconds)
    ...
    -- Ensure boxRegion is scaled if it's a fixed-size region
	boxRegion = boxRegion or Region(0, 0, screen.x, screen.y)
	return boxRegion:exists(PatternBuilder, waitTime) -- Use the adjusted waitTime
end
...
function SearchImageNew(target, boxRegion, maxScore, Color, Mask, Time)
	...
	Time = Time or 3 -- Default to 3 seconds if not explicitly provided
    Color = Color or false
    Mask = Mask or false
    maxScore = maxScore or 0.9
	local TImage = target
	if not (typeOf(target) == "table") then TImage = {target} end
	...
	repeat
		for i, t in ipairs(TImage) do
			...
			if (Cur_Image) then
				local Cur_Score_Val = Cur_Image:getScore() -- Renamed Cur_Score to Cur_Score_Val to avoid conflict
				if (Cur_Score_Val > maxScore) then
					...
					maxScore = Cur_Score_Val
					result = {x = X, y = Y, xy = XY, name = get_file_name(TImage[i]), score=maxScore, w = W, h = H, r = R, sx = SX, sy = SY, loc=Location(X,Y)}
				end
			else
				Cur_Image = nil
			end
            if result.name then break end -- Found an image, no need to check others in this iteration
		end
	until(result.name) or (Search_Timer:check() > Time) -- Use the adjusted Time parameter
	Search_Timer = nil
...
function Claim_Daybreak_Freebie()
    ...
    local buyFound, buyBtn = WaitExists("buy.png", 4, nil, 0.9, true)
    if not (buyFound and buyBtn and buyBtn.xy) then
    ...
    Logger("Claim_Daybreak_Freebie: searching free.png (full screen)")
    local freeRegion = Region(0, 0, screen.x, screen.y)
    local freeFound, freeFull = WaitExists("free.png", 4, freeRegion, 0.9, true)
    if not (freeFound and freeFull and freeFull.xy) then
        Logger("Claim_Daybreak_Freebie: free.png not found - returning to Island view")
        local backFound, backBtn = WaitExists("trek/back.png", 3, Upper_Left, 0.9, true)
        if backFound and backBtn and backBtn.xy then
            Press(backBtn.xy, 1)
            wait(0.3)
        end
        return false
    end
    Logger("Claim_Daybreak_Freebie: free.png found, clicking")
    Press(freeFull.xy, 1)
    wait(0.5)

    Logger("Claim_Daybreak_Freebie: searching free1.png (Upper_Half)")
    local confirmFound, freeConfirm = WaitExists("free1.png", 3, Upper_Half, 0.9, true)
    if confirmFound and freeConfirm and freeConfirm.xy then
    ...
    Logger("Claim_Daybreak_Freebie: tapping Back (trek/back.png)")
    local backFound, backBtn = WaitExists("trek/back.png", 3, Upper_Left, 0.9, true)
    if backFound and backBtn and backBtn.xy then
        Press(backBtn.xy, 1)
        wait(0.3)
    ...
    WaitExists("Island Storehouse.png", 3, Lower_Left, 0.9, true)
...
function expert(button, opts)
	...
	if (romulus and romulus.xy) then
		Logger("Expert: Rmrk.png found, tapping")
		Press(romulus.xy, 1)
		wait(3)
		PressRepeatNew("Side Opened.png", "Side Closed.png", 1, 2, nil, nil, 0.9, true, true)
		return true, 0
	...
	Logger("Expert: Rmrk.png not found; switching to world")
...
function Dawn_Academy()
    Current_Function = getCurrentFunctionName()
    Logger("Dawn Academy: Start")

    ----------------------------------------------------------------------
    -- Config
    ----------------------------------------------------------------------
    local SIM_DEFAULT   = 0.90
    local TIMEOUT_SHORT = 3      -- seconds
    local TIMEOUT_MED   = 7
    local TIMEOUT_LONG  = 10

    -- Fallback regions only if truly nil
    if Upper_Left == nil or Upper_Right == nil or Lower_Left == nil then
        local function pctRegion(px, py, pw, ph)  -- 0..1
            return Region(
                math.floor(screen.x * px),
                math.floor(screen.y * py),
                math.floor(screen.x * pw),
                math.floor(screen.y * ph)
            )
        end
        Upper_Left  = Upper_Left  or pctRegion(0.00, 0.00, 0.50, 0.50)
        Upper_Right = Upper_Right or pctRegion(0.50, 0.00, 0.50, 0.50)
        Lower_Left  = Lower_Left  or pctRegion(0.00, 0.50, 0.50, 0.50)
    end

    local successfullyEnteredDawnAcademy = false
    local claimedReward = false
    local trekBackBtnLoc = nil  -- Location (xy) of back button

    ----------------------------------------------------------------------
    -- Local helpers (NO type() calls)
    ----------------------------------------------------------------------
    local function find(img, region, sim, timeout)
        sim = sim or SIM_DEFAULT
        timeout = timeout or TIMEOUT_SHORT
        local t0 = os.time()
        local hit
        while (os.time() - t0) < timeout do
            hit = SearchImageNew(img, region, sim, false, false, 1)
            if hit and hit.name then return hit end
            wait(0.3)
        end
        return nil
    end

    local function tapIf(img, region, sim, timeout)
        local r = find(img, region, sim, timeout)
        if r and r.xy then
            Press(r.xy, 1)
            return true, r
        end
    ...
    local function swipeUp(times)
        times = times or 2
        for i = 1, times do
            swipe(
                Location(ranNumbers and ranNumbers(6, 2) or 6,   ranNumbers and ranNumbers(845, 5) or 845),
                Location(ranNumbers and ranNumbers(6, 2) or 6,   ranNumbers and ranNumbers(10, 5)  or 10),
                0.5
            )
            wait(0.4)
        end
        swipe(
            Location(ranNumbers and ranNumbers(6, 2) or 6, ranNumbers and ranNumbers(845, 5) or 845),
            Location(ranNumbers and ranNumbers(0, 0) or 0, ranNumbers and ranNumbers(0, 0)  or 0),
            0.1
        )
        wait(0.2)
    end

    local function ensureCity(maxAttempts)
        maxAttempts = maxAttempts or 3
        for i = 1, maxAttempts do
            local city = find("City.png", nil, 0.88, 2)
            if city then
                Logger("Dawn Academy: City detected.")
                return true
            end
            Logger("Dawn Academy: City not detected, attempt " .. i .. " -> try close/back.")
            local closed = tapIf("trek/close.png", Upper_Right, 0.88, 1)
            if not closed then
                Go_Back("Dawn Academy: Returning to City (attempt " .. i .. ")")
            else
                wait(0.6)
            end
        end
    ...
    -- 1) Open side
    ----------------------------------------------------------------------
    Logger("Dawn Academy: [1/6] Open side panel")
    if not Side_Check_Opener() then
        Logger("Dawn Academy: Side panel not accessible.")
        Go_Back("Dawn Academy: Abort - side panel failed.")
        return false, false
    end
    ...
    -- 2) Swipe
    ----------------------------------------------------------------------
    Logger("Dawn Academy: [2/6] Swipe to reveal Trek")
    swipeUp(2)

    ----------------------------------------------------------------------
    -- 3) Click Trek (then PROVE we are inside by finding back.png)
    ----------------------------------------------------------------------
    Logger("Dawn Academy: [3/6] Click 'trek/trek.png'")
    -- We don't assume the return type of PressRepeatNew; just use it to navigate
    PressRepeatNew("trek/trek.png", "trek/back.png", 1, TIMEOUT_LONG, Lower_Left, Upper_Left, 0.90)

    -- Now *actively* find the back button to confirm and capture coordinates
    local backBtn = find("trek/back.png", nil, 0.90, 4)
    if backBtn and backBtn.xy then
        successfullyEnteredDawnAcademy = true
        trekBackBtnLoc = backBtn.xy
        Logger("Dawn Academy: Entered Dawn Academy (back button visible).")
    else
        Logger("Dawn Academy: Failed to enter Dawn Academy (back not visible).")
        Go_Back("Dawn Academy: Abort - couldn't enter Trek screen.")
        return false, false
    end
    ...
    -- 4) Click Bag
    ----------------------------------------------------------------------
    Logger("Dawn Academy: [4/6] Click 'trek/bag.png'")
    local bagBtn = find("trek/bag.png", Upper_Right, 0.80, TIMEOUT_MED)
    if bagBtn and bagBtn.xy then
        Press(bagBtn.xy, 1)
        wait(0.8)
    else
        Logger("Dawn Academy: Bag not found; continuing.")
    end

    ----------------------------------------------------------------------
    -- 5) Find claim and click; if not found then close and back
    ----------------------------------------------------------------------
    Logger("Dawn Academy: [5/6] Look for 'trek/claimtrek1.png'")
    local claimBtn = find("trek/claimtrek1.png", nil, 0.90, TIMEOUT_MED)
    if claimBtn and claimBtn.xy then
        Logger("Dawn Academy: Claim found. Clicking.")
        Press(claimBtn.xy, 1)
        claimedReward = true
        if updateDawnAcademyLastClaim then updateDawnAcademyLastClaim(os.time()) end
        wait(1.2)
        -- close reward popup if shown
        tapIf("trek/close.png", Upper_Right, 0.88, 2)
        wait(0.6)
    else
        Logger("Dawn Academy: No claim button. Try closing any open panel.")
        tapIf("trek/close.png", Upper_Right, 0.88, 2)
        wait(0.5)
    end

    -- Back out of Dawn Academy using stored back if available
    if trekBackBtnLoc then
        Logger("Dawn Academy: Press Dawn Academy back.")
        Press(trekBackBtnLoc, 1)
        wait(0.8)
    else
        Logger("Dawn Academy: No stored back location; using Go_Back to exit.")
        Go_Back("Dawn Academy: Exit from Trek")
    end

    ----------------------------------------------------------------------
    -- 6) Verify City; if not, try close and back again
    ----------------------------------------------------------------------
    Logger("Dawn Academy: [6/6] Verify City and clean up")
    local inCity = ensureCity(3)
    if not inCity then
        Logger("Dawn Academy: City still not visible; one last Go_Back.")
        Go_Back("Dawn Academy: Ensuring City")
        inCity = (find("City.png", nil, 0.88, 2) ~= nil)
    end

    Logger(string.format("Dawn Academy: Finished. Entered=%s, Claimed=%s",
        tostring(successfullyEnteredDawnAcademy), tostring(claimedReward)))
    return successfullyEnteredDawnAcademy, claimedReward
end
...
function luckyPouch()
    ...
    -- Scale the region for the button if it's fixed
	if (SingleImageWait(LPDir.. "Lucky Pouch.png", 0, Lower_Half, 0.9, true)) then
        -- Scale the location for clicking
		PressRepeatHexColor(Location(610, 1380), Location(682, 39), "#CFF4F5", 5, 3)
		while true do
            -- Use SearchImageNew with a timeout for better detection
			local luckyPouchChat = SearchImageNew(LPDir.. "Lucky Pouch.png", Lower_right, 0.9, true, false, 1) -- Add a timeout
			if (luckyPouchChat.name) then
				Press(luckyPouchChat.xy, 1) -- Click the identified chat icon
				local LPTimer = SearchImageNew(LPDir.. "Lucky Pouch Timer.png", Upper_Half, 0.9, true, false, 2) -- Add a timeout
				if (LPTimer.name) then
                    -- If LPTimer has a specific ROI for the reward, scale it
					PressRepeatNew(LPTimer.xy, LPDir.. "Lucky Pouch Reward.png", 1, 3)
                    -- Scale the location for closing the reward
					PressRepeatHexColor(Location(320, 30), Location(682, 39), "#CFF4F5", 5, 1)
				end
			else break end -- Lucky Pouch chat icon is no longer visible
		end
