...
function Main_GUI(version)
	...
	newRow()
        ...
        newRow()
        addCheckBox("Experts_Options", "Experts", false)
        addSeparator()
        newRow()
...
function rallyStarter(Rally, useCounter)
    ...
    -- *** NEW DEBUG LOG HERE ***
    Logger(string.format("DEBUG: In rallyStarter before calling Auto_Beast. Attack_Type: %s, Flag_Req: %s, Use_Hero: %s", Attack_Type, Flag_Req, tostring(Use_Hero)))
...
function Dawn_Academy()
    Current_Function = getCurrentFunctionName()
    Logger("Opening Dawn Academy based on user-defined logic.")

    local successfullyEnteredDawnAcademy = false
    local claimedReward = false
    local trekBackBtnLoc = nil -- To store the location of the 'back' button on the Dawn Academy screen

    -- 1. Open side button
    if not Side_Check_Opener() then
        Logger("Failed to open side panel. Cannot proceed with Dawn Academy.")
        Go_Back("Dawn Academy: Side panel not accessible.")
        return false, false -- (opened, claimed)
    end

    -- 2. Swipe up to reveal the Trek button
    Logger("Performing swipes to reveal Trek button.")
    swipe(Location(ranNumbers(6, 2), ranNumbers(845, 5)), Location(ranNumbers(6, 2), ranNumbers(10, 5)), 0.5)
    wait(0.5)
    swipe(Location(ranNumbers(6, 2), ranNumbers(845, 5)), Location(ranNumbers(6, 2), ranNumbers(10, 5)), 0.5)
    wait(0.5)
    swipe(Location(ranNumbers(6, 2), ranNumbers(845, 5)), Location(ranNumbers(0, 0), ranNumbers(0,0)), 0.1) -- Small swipe to settle UI

    -- 3. Click Trek button and confirm entry to Dawn Academy
    Logger("Attempting to click Trek Btn to enter Dawn Academy.")
    local trekEntryConfirmation = PressRepeatNew("trek/trek.png", "trek/back.png", 1, 5, Lower_Left, Upper_Left, 0.9)

    if trekEntryConfirmation.name == "back" then
        Logger("Successfully entered Dawn Academy screen.")
        successfullyEnteredDawnAcademy = true
        trekBackBtnLoc = trekEntryConfirmation.xy -- Store the location of the back button for later use
    else
        Logger("Failed to find 'trek/back.png' after clicking Trek button. Dawn Academy entry failed.")
        -- Attempt to go back, as we might be stuck on a half-loaded screen or wrong screen
        Go_Back("Dawn Academy: Trek button not found or entry failed.")
        return false, false -- (opened, claimed)
    end

    -- 4. Find and click the Bag button
    Logger("Searching for 'trek/bag.png'.")
    local bagBtn = SearchImageNew("trek/bag.png", Upper_Right, 0.8, false, false, 1) -- Short wait as it should be visible if we are on screen
    if (bagBtn.name) then
        Logger("Bag Btn Found, Clicking.")
        Press(bagBtn.xy, 1)
        wait(2)
        
        -- After clicking bag, a close button typically appears to exit the bag menu
        local bagClose = SearchImageNew("trek/close.png", Upper_Right, 0.8, false, false, 2) -- Give it a bit more time to appear
        if (bagClose.name) then
            Logger("Found bag close button, clicking it.")
            Press(bagClose.xy, 1)
            wait(0.5)
        else
            Logger("No specific close button found after clicking bag, assuming it closed automatically or didn't open a sub-menu.")
        end
    else
        Logger("Bag button not found on Dawn Academy screen.")
    end

    -- 5. After dealing with the Bag, check for Claim button (`claimtrek1.png`)
    Logger("Now checking for (Claim button).")
    local claimBtn = SearchImageNew("trek/claimtrek1.png", Upper_Right, 0.9, false, false, 2) -- Give it 2 seconds to appear
    if (claimBtn.name) then
        Logger("Claim Found, Clicking.")
        Press(claimBtn.xy, 1)
        claimedReward = true
        -- Call your persistence function here to mark as claimed for today
        -- For example: updateDawnAcademyLastClaim(os.time()) -- Uncomment if you use this
        wait(0.5)
        
        -- After claiming, there's often a rewards popup that needs closing
        local claimRewardsClose = SearchImageNew("trek/close.png", Upper_Right, 0.89, false, false, 2)
        if (claimRewardsClose.name) then
            Logger("Found claim rewards close button, clicking it.")
            Press(claimRewardsClose.xy, 1)
            wait(0.5)
        else
            Logger("No specific close button found after claiming reward.")
        end
    else
        Logger("Claim Button Not Found.")
    end

    -- 7. Universal Cleanup: Click close, then back, then ensure City.png exists
    Logger("Executing universal cleanup: Close Dawn Academy, go back to city.")
    
    -- First, click a general 'close' button if one is present on the Dawn Academy screen
    -- This handles exiting the main Dawn Academy UI if no sub-menus were open or left open
    local generalCloseBtn = SearchImageNew("trek/close.png", Upper_Right, 0.89, false, false, 1)
    if generalCloseBtn.name then
        Logger("Found general Dawn Academy close button, clicking it.")
        Press(generalCloseBtn.xy, 1)
        wait(0.5)
    else
        Logger("No general Dawn Academy close button found (might already be closed or UI is different).")
    end

    -- Use the stored back button location if possible, otherwise rely on generic keyevent(4) handled by Go_Back
    if trekBackBtnLoc then
        Logger("Using stored 'back' button location to exit Dawn Academy.")
        Press(trekBackBtnLoc, 1)
        wait(0.5) -- Give time for transition
    else
        Logger("Stored 'back' button location not available. Relying on Go_Back for navigation.")
    end

    -- Finally, call Go_Back to ensure we are securely on the City screen
    -- Go_Back already handles multiple keyevent(4) presses and checks for City.png
    Go_Back("Dawn Academy cleanup completed, ensuring return to City screen.")

    Logger(string.format("Dawn Academy function finished. Successfully entered: %s, Claimed reward: %s", tostring(successfullyEnteredDawnAcademy), tostring(claimedReward)))
    return successfullyEnteredDawnAcademy, claimedReward -- Return status: (opened, claimed)
end
...
function RunScript(version)
	...
	User_ID = getUserID()
        ...
        if (Experts_Options) then Experts_GUI() end
